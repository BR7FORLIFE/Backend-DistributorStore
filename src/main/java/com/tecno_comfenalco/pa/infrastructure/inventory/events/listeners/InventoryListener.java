...
public class InventoryListener {
    ...
    public void handleDiffInventoryByOrder(InventoryDiffEvent inventoryDiff) {
        inventoryDiff.orderProducts().stream().forEach(order -> {
            updateStock(inventoryDiff.distributorId(), order.getProductId(), order.getQuantity(), inventoryDiff.op());
        });
    }
    ...
    @Async
    @EventListener
    public void handleSumStockInInventory(ProductDeletedEvent productDeletedEvent) {

    }

    public void updateStock(UUID distributorId, UUID productId, Long delta, OP op) {
